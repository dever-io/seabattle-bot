# fix-5991286ffd5b9d14 — No automatic win detection in attack grid flow

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 SBOT

The task spec says *"When a player's ships are all sunk, send final results, update ratings, and offer 'Rematch' and 'View replay' buttons."* However, the attack grid handler (`src/handlers/E7T1.ts`) never calls `checkWinCondition()` after a `SHOT_RESULT_SUNK` outcome. After sinking the last enemy ship via the grid, the game shows no end screen — the user must manually type `/endgame` to see results. The detection is gated on a manual command instead of being triggered automatically when the win condition is met.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-5991286ffd5b9d14.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-5991286ffd5b9d14.json` (and any new command `tests/commands/fix-5991286ffd5b9d14.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-5991286ffd5b9d14.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-5991286ffd5b9d14.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
