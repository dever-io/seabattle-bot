# fix-61d76119a4a7b6db — E9T4: missing command manifest tests/commands/E9T4.json

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 SBOT

The E9T4 handler (`src/handlers/E9T4.ts`) defines the commands `/fire` and `/history`. Per the E9T4 task spec: "If this task adds a bot command, declare it in its OWN file `tests/commands/E9T4.json`". This file does not exist.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-61d76119a4a7b6db.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-61d76119a4a7b6db.json` (and any new command `tests/commands/fix-61d76119a4a7b6db.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-61d76119a4a7b6db.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-61d76119a4a7b6db.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
