# fix-32a68c0122968951 — E4T1/E4T2: Test file names deviate from task spec requirements

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 SBOT

E4T1 spec requires `tests/specs/matchmaking.json` — actual file is `tests/specs/E4T1.json`. E4T2 spec requires `tests/specs/matchmaking-expansion.json` — actual file is `tests/specs/E4T2.json`. The tests-gate globs all spec files, so the tests still run, but this is a spec deviation.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-32a68c0122968951.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-32a68c0122968951.json` (and any new command `tests/commands/fix-32a68c0122968951.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-32a68c0122968951.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-32a68c0122968951.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
