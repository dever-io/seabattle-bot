# fix-6f808e69bd6116c2 — E4T1: Redis data structure type mismatch — `sadd` (SET) vs `rpop` (LIST) on same key

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 SBOT

In `src/handlers/E4T1.ts` line 53, the user is added to the matchmaking queue via `redis.sadd(QUEUE_KEY, …)` which creates the Redis key `matchmaking:queue` as a **SET**. Then at line 59, the code calls `redis.rpop(QUEUE_KEY)` — `rpop` is a **LIST** command. Redis will reject this with a `WRONGTYPE` error because the key already holds a SET data type. The opponent-finding path (`opponentStr && opponentStr !== currentChatStr`, lines 62–88) is **unreachable at runtime** — matches can never be found through the queue. E1T1 also writes to this same key via `sadd`, guaranteeing it will be a SET. The entire Quick Match matchmaking flow is broken.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-6f808e69bd6116c2.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-6f808e69bd6116c2.json` (and any new command `tests/commands/fix-6f808e69bd6116c2.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-6f808e69bd6116c2.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-6f808e69bd6116c2.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
