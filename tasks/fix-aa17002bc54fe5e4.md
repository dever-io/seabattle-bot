# fix-aa17002bc54fe5e4 — E1T1: misleading matchmaking status when Redis unavailable

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 SBOT

When `REDIS_URL` is not set, the local `getRedisClient()` returns `null`, skipping the `sadd` call that adds the user to `matchmaking:queue`. However, `editMessageText("Looking for a match...")` still fires, misleading the user into thinking they are queued for matchmaking when they are not. The handler should guard against a null Redis client by replying with an error ("Matchmaking is temporarily unavailable") or by importing and using the toolkit's shared `getRedisClient()` from `../storage/persistent.js` and checking the result before editing the message.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-aa17002bc54fe5e4.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-aa17002bc54fe5e4.json` (and any new command `tests/commands/fix-aa17002bc54fe5e4.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-aa17002bc54fe5e4.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-aa17002bc54fe5e4.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
