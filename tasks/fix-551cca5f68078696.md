# fix-551cca5f68078696 — /start command handler conflict between E2T1.ts and start.ts

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 SBOT

Both `src/handlers/E2T1.ts` and `src/handlers/start.ts` register `command("start")` handlers. `buildBot()` auto-loads both via `bot.use()`, so in grammY's middleware pipeline **both handlers fire on every `/start` message**, producing duplicate welcome messages:

1. E2T1.ts sends: *"Welcome, $name! Your profile has been created.\nReady to get started?"* (with "Get Started" inline keyboard)
2. start.ts sends: *"Welcome! I am ready to help."* (with Help/Status menu keyboard)

The test harness's non-strict matching mode (allowing incidental extra calls) masks this bug — both test specs pass in isolation because the expected call is found as a subsequence among all captured calls. In production, users see two conflicting welcome messages.

`tests/specs/start.json` also mirrors E2T1's expected output rather than start.ts's own behavior, further hiding the conflict.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-551cca5f68078696.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-551cca5f68078696.json` (and any new command `tests/commands/fix-551cca5f68078696.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-551cca5f68078696.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-551cca5f68078696.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
