# fix-51693cfb2c3853ad — end:replay callback is a stub

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 SBOT

The `end:replay` callback (`src/handlers/E8T1.ts:94-99`) replies "Replay viewer coming soon. Check your match history with /history." This is a stub — no replay viewer is implemented, and the referenced `/history` command does not exist anywhere in the codebase. Violates the implementation contract: *"Stubbed code: empty bodies, TODO/FIXME, commented-out logic, or `throw new Error("not implemented")`."* "Coming soon" is a TODO-in-response.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-51693cfb2c3853ad.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-51693cfb2c3853ad.json` (and any new command `tests/commands/fix-51693cfb2c3853ad.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-51693cfb2c3853ad.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-51693cfb2c3853ad.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
