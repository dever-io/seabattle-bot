# fix-83cfe10fc3929a9d — end:rematch callback is a stub

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 SBOT

The `end:rematch` callback (`src/handlers/E8T1.ts:87-92`) replies "Rematch requested! Set up a new game with /invite or /quickmatch." This does not actually create a rematch — it is a canned response directing the user to manually issue other commands. No rematch logic (re-seeding a board, resetting the session for a new round with the same opponent) is implemented. Violates the implementation contract against stubs.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-83cfe10fc3929a9d.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-83cfe10fc3929a9d.json` (and any new command `tests/commands/fix-83cfe10fc3929a9d.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-83cfe10fc3929a9d.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-83cfe10fc3929a9d.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
